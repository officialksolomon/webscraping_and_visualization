...
class DataVisualizer(ABC):
    ...
    @abstractmethod
    def validate_data_value(self, data):
        pass
...
class PieChartVisualizer(DataVisualizer):
    ...
    def validate_data_value(self, data):
        if type(int(data)) not in [int, float]:
            raise ValueError("Invalid Value. Pie Chart requires int or float")
        return data
    ...
    def visualize_data(self):
        ...
        if self.data is not None:
            ax.pie(
                [self.validate_data_value(data.replace(",", "")) for data in self.data],
                labels=self.data_columns,
            )
            ax.set_title(
                "Coronavirus cases, deaths, and recovered cases worldwide."
            )
